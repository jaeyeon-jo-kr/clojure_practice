(ns 164_language_as_a_dfa)

;;;;
(comment "A deterministic finite automaton (DFA) is an abstract machine that recognizes a regular language. Usually a DFA is defined by a 5-tuple, but instead we'll use a map with 5 keys:
...
;;current->accepts
(defn dd [{:keys [states alphabet start accepts transitions]}]
  [states alphabet start accepts transitions])


;;(if (contains? accepts alph)
;                        (conj str-set (str alph))
;                        (into str-set (map #(str alph %) (exp next-state))))

(fn [{:keys [states alphabet start accepts transitions]}]
  (letfn [(find-seq [str-set q accepted]
            (iterate (fn [[str-set q nexts]]
                       (let [[a nq] (get transitions q)
                             str-set (if (contains? accepts a)])
                         )
                     [#{} start])))]))
